...
class PreInscripcionDiarioExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithStyles
{
    protected $dates;
    ...
    public function collection()
    {
        ...
        // Obtener las preinscripciones agrupadas por programa y fecha
        $preinscripciones = PreInscripcion::selectRaw('programas.grado_id, pre_inscripcions.programa_id, DATE(pre_inscripcions.created_at) as date, COUNT(*) as count')
            ->join('programas', 'pre_inscripcions.programa_id', '=', 'programas.id')
            ->join('grados', 'programas.grado_id', '=', 'grados.id')
            ->groupBy('programas.grado_id', 'pre_inscripcions.programa_id', 'date')
            ->orderByRaw("
            CASE 
                WHEN grados.nombre = 'MAESTRÍA' THEN 1
                WHEN grados.nombre = 'DOCTORADO' THEN 2
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->setCellValue('A1', 'REPORTE DE PRE-INSCRIPCION | ESCUELA DE POSGRADO - ADMISION ' . $periodo);
