...
class InscripcionDiarioFacultadExport implements FromArray, WithEvents, ShouldAutoSize, WithStyles
{
    protected $dates;
    ...
    public function array(): array
    {
        ...
        foreach ($facultads as $facultad) {
            ...
            foreach ($this->sortedGrados as $gradoNombre) {
                ...
                foreach ($programas as $programa) {
                    ...
                    foreach ($this->dates as $date) {
                        $count = Inscripcion::where('programa_id', $programa->id)
            ...
            $totalRow = ['', '', 'TOTAL'];
            foreach ($this->dates as $date) {
                $sumPerDate = Inscripcion::whereHas('programa', function ($query) use ($facultad) {
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue("A1", "REPORTE DE INSCRIPCION POR FACULTAD | ESCUELA DE POSGRADO - ADMISIÓN $periodo");
